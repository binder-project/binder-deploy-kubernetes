...
Collection.prototype.find = function (template, cb) {
  ...
  var query = function (err, coll) {
    self.logger.debug('searching for {0} in collection: {1}'.format(JSON.stringify(template), this.name))
    if (err) {
      return cb(err)
    }
    if (template) {
...
Collection.prototype.findOne = function (template, cb) {
  ...
  var query = function (err, coll) {
    ...
    self.logger.debug(logString.format(JSON.stringify(template), this.name))
    if (err) {
      return cb(err)
    }
    if (template) {
...
Collection.prototype.insert = function (model, cb) {
  this.logger.debug('adding {0} to collection: {1}'.format(JSON.stringify(model), this.name))
  var self = this
  ...
  var handleCreate = function (next) {
    if (model.create) {
      model.create(function (err) {
        if (err) {
          self.logger.error('model creation failed')
          return next(err)
        }
        return next(null)
      })
  ...
  var handleDb = function (next) {
    if (self.db) {
      self.db.insert(model, function (err) {
        if (err) {
          self.logger.error('db insertion failed')
          return next(err)
        }
        return next(null)
      })
  ...
  async.series([
    ...
    self.logger.info('after the async parallel for model {0} with err: {1}!'.format(JSON.stringify(model), err))
    if (err) {
      self.logger.error('could not insert model {0} into collection {1}, err: {2} '.format(JSON.stringify(model, self.name, err)))
...
Collection.prototype.remove = function (template, cb) {
  ...
  var removeItem = function (err) {
    self.logger.info('removing {0} from collection: {1}'.format(JSON.stringify(template), this.name))
    if (err) {
...
Collection.prototype.update = function (template, update, cb) {
  var self = this
  self.logger.info('updating {0} with properties {1} in collection {2}'.format(JSON.stringify(template),
