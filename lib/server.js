...
BinderDeployKubernetes.prototype._makeBinderAPI = function () {
  return {
    ...
    status: this.cluster.getApplication.bind(this.cluster),
    statusAll: this.cluster.getAllApplications.bind(this.cluster),
    _preload: this.cluster.preload.bind(this.cluster),
    _preloadStatus: this.cluster.preloadStatus.bind(this.cluster)
  }
...
BinderDeployKubernetes.prototype._start = function (cb) {
  ...
  this.backgroundTasks = [
    ...
    {
      name: 'binder-deploy-health-check',
      script: path.join(__dirname, '../scripts/health-check.js'),
      silent: true
    }
  ]
  ...
  var initialize = function (err, conn) {
    ...
    var pc = proxyClient(self.opts)
    pc.launchClusterProxy(function (err) {
      if (err) return cb(err)
