...
function BinderDeployKubernetes (opts) {
  ...
  BinderDeployKubernetes.super_.call(this, opts)
  this.kubePort = opts.kube.proxyPort
}
 ...
 */
BinderDeployKubernetes.prototype._start = function () {
  ...
  this.cluster = new ClusterManager(this.opts)
  this.pools = new PoolManager(this.opts)
  this.server = this._createServer()

  // Initialize the singleton proxy client here (any special options here)
  proxyClient(this.opts)
  // Initialize the singleton Kubernetes client here
  kubeClient(this.opts)
}

/**
 * Performs all module-specific stopping behavior
 */
BinderDeployKubernetes.prototype._stop = function () {
  shell.exec(['pm2', 'stop', 'binder-kubernetes-proxy'])
}
