...
ProxyClient.prototype.registerProxyRoute = function (route, target, cb) {
  ...
  function retryRegistration (cb) {
    async.retry({ times: 10, interval: 1000 }, function (next) {
      register.bind(self)(route, target, next)
    }, function (err) {
      if (err) self.logError('FINAL -- could not register proxy route ' + route + ' after retrying')
      return cb(err)
    })
  }
  if (!this.token) {
  ...
  var remove = function (route, cb) {
    ...
    request(opts, function (err, response, body) {
      ...
      if (response.statusCode !== 204) {
        var msg = 'could not remove proxy route -- bad response: {0}'.format(JSON.stringify(response))
        self.logError(msg)
        return cb(msg)
      }
      return cb(null)
