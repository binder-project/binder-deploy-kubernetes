...
ProxyClient.prototype._startPod = function (token, cb) {
      ...
      containers: [
        {
          ...
          command: ['configurable-http-proxy', '--port', '8080', '--error-path', '/home/main/error_pages',
            '--api-port', '8081', '--api-ip', '0.0.0.0', '--log-level', 'debug', '--no-include-prefix'],
          ports: [
            {
              containerPort: 8080,
              protocol: 'TCP'
  ...
  this.client.pods.changeState({
    ...
    if (err) {
      self.logger.info('could not start proxy pod: ' + err)
    } else {
...
ProxyClient.prototype._startService = function (name, service, cb) {
  ...
  this.client.services.changeState({
    ...
    if (err) {
      self.logger.info('could not start service {0}: '.format(name) + err)
      return cb(err)
    }
...
ProxyClient.prototype._getRunningInfo = function (cb) {
  ...
  async.series([
    ...
    if (err) {
      self.logger.error('error in _getRunningInfo: ' + err)
      return cb(err)
    }
    var info = {
      lookupIP: results[1],
    ...
    self.logger.info('in _getRunningInfo, info: {0}'.format(JSON.stringify(info)))
    return cb(null, info)
  })
}
...
ProxyClient.prototype.launchClusterProxy = function (cb) {
  ...
  this._getRunningInfo(function (err, info) {
    ...
    self.logger.info('cluster proxy not fully running -- starting launch')
    var token = hat()
    ...
    async.parallel(tasks, function (err, ips) {
      if (err) {
        self.logger.error('in launchClusterProxy: ' + err)
        return cb(err)
      }      
      self.lookupIP = ips[2]
...
ProxyClient.prototype.removeProxyRoute = function (route, cb) {
  ...
  var remove = function (route, cb) {
    ...
    request(opts, function (err, response, body) {
      if (err) {
        self.logger.error('could not remove proxy route: ' + err)
        return cb(err)
      }
